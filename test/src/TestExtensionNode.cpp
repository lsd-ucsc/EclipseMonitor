...
GTEST_TEST(TestExtensionNode, SingleExtensionTest)
{
    ...
    std::unique_ptr<Leaf::LeafNode> leaf = Leaf::LeafNode::NewLeafNodeFromNibbles(nibbles, val);

    std::unique_ptr<Branch::BranchNode> branchNode = SimpleObjects::Internal::make_unique<Branch::BranchNode>();
    branchNode->SetBranch(0, std::move(leaf));
    ...
    Extension::ExtensionNode extensionNode(std::move(nibs), std::move(branchNode));
...
GTEST_TEST(TestExtensionNode, NestedExtensionTest)
{
    ...
    std::unique_ptr<Leaf::LeafNode> leaf1 = Leaf::LeafNode::NewLeafNodeFromNibbles(nibbles1, val1);

    std::unique_ptr<Branch::BranchNode> branchNode1 = SimpleObjects::Internal::make_unique<Branch::BranchNode>();
    branchNode1->SetBranch(0, std::move(leaf1));
    branchNode1->SetValue(SimpleObjects::Bytes({'b'}));

    std::vector<uint8_t> nibbleBytes1 = {1, 2};
    std::vector<Nibble> nibs1 = NibbleHelper::FromBytes(nibbleBytes1);
    std::unique_ptr<Extension::ExtensionNode> extensionNode1 = SimpleObjects::Internal::make_unique<Extension::ExtensionNode>(std::move(nibs1), std::move(branchNode1));

    std::vector<Nibble> nibbles2 = {7, 7, 7};
    SimpleObjects::Bytes val2 = {'d'};
    std::unique_ptr<Leaf::LeafNode> leaf2 = Leaf::LeafNode::NewLeafNodeFromNibbles(nibbles2, val2);

    std::unique_ptr<Branch::BranchNode> branchNode2 = SimpleObjects::Internal::make_unique<Branch::BranchNode>();
    branchNode2->SetBranch(4, std::move(extensionNode1));
    branchNode2->SetBranch(8, std::move(leaf2));
    branchNode2->SetValue(SimpleObjects::Bytes({'e'}));

    std::vector<uint8_t> nibbleBytes2 = {3, 4};
    std::vector<Nibble> nibs2 = NibbleHelper::FromBytes(nibbleBytes2);
    Extension::ExtensionNode extensionNode2(std::move(nibs2), std::move(branchNode2));
