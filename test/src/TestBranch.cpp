...
GTEST_TEST(TestBranch, TestOneLeaf)
{
    ...
    std::unique_ptr<TrieNode::NodeBase> leafNode = Leaf::LeafNode::NewLeafNodeFromNibbles(nibbles, val);
...
GTEST_TEST(TestBranch, TestTwoLeaves)
{
    ...
    std::unique_ptr<TrieNode::NodeBase> leafNode1 = Leaf::LeafNode::NewLeafNodeFromNibbles(nibbles1, val1);

    std::vector<Nibble> nibbles2 = {1, 2, 3, 4, 5};
    SimpleObjects::Bytes val2 = {'w', 'a', 't', 'e', 'r'};
    std::unique_ptr<TrieNode::NodeBase> leafNode2 = Leaf::LeafNode::NewLeafNodeFromNibbles(nibbles2, val2);
...
GTEST_TEST(TestBranch, TestNestedBranch)
{
    ...
    branchNode1->SetBranch(0, std::move(leafNode));
    branchNode1->SetValue(SimpleObjects::Bytes({'v', 'e', 'r', 'b'}));

    Branch::BranchNode branchNode2;
